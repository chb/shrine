...
final class PanelTest extends TestCase with AssertionsForJUnit with ShouldMatchers {
  ...
  private def now = QueryDefinition.truncateDate(Utils.now)
  
  private val startDate = Some(now)
  private val endDate = Some(now)
  
  private val p3 = Panel(3, true, 99, startDate, endDate, Seq(term1, term2, term3))
