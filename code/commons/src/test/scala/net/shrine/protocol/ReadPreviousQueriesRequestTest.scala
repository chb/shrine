...
class ReadPreviousQueriesRequestTest extends ShrineRequestValidator {
  ...
  @Test
  def testToXml {
    ReadPreviousQueriesRequest(projectId, waitTimeMs, authn, username, fetchSize).toXml should equal(readPreviousQueriesRequest)
  }

  @Test
  def testToI2b2 {
    ReadPreviousQueriesRequest(projectId, waitTimeMs, authn, username, fetchSize).toI2b2 should equal(request)
  }
