...
final class QueryDefinitionTest extends TestCase with AssertionsForJUnit with ShouldMatchers {
  ...
  def testToI2b2 {
    ...
    val elem = q1.toI2b2.asInstanceOf[Elem]
    
    elem.prefix should equal(QueryDefinition.i2b2NamespacePrefix)
    elem.namespace should equal(QueryDefinition.i2b2Namespace)
  }
