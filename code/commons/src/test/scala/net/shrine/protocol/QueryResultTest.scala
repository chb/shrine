 ...
 */
final class QueryResultTest extends TestCase with AssertionsForJUnit with ShouldMatchersForJUnit with I2b2SerializableValidator{
  //TODO
  ...
  def testToI2b2() = {
    ...
    val resultId = 1L
    val instanceId = 2L
    val resultType = ResultOutputType.PATIENTSET
    val setSize = 12L
    ...
    queryResult.toI2b2String should equal (XmlUtil.stripWhitespace(
  ...
  def testToI2b2WithErrors {
    ...
    actual should equal (expected) //compare strings because scala xml comparison isn't working right here
