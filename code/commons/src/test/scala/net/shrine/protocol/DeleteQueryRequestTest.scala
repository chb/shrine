...
class DeleteQueryRequestTest extends ShrineRequestValidator {
  ...
  def testShrineRequestFromI2b2 {
    assertTrue(CrcRequest.fromI2b2(request).isInstanceOf[DeleteQueryRequest])
  }

  @Test
  def testDoubleDispatchingShrineRequestFromI2b2 {
    assertTrue(HandleableShrineRequest.fromI2b2(request).isInstanceOf[DeleteQueryRequest])
  }

  @Test
  def testToXml {
    DeleteQueryRequest(projectId, waitTimeMs, authn, queryId).toXml should equal(deleteQueryRequest)
  }

  @Test
  def testToI2b2 {
    DeleteQueryRequest(projectId, waitTimeMs, authn, queryId).toI2b2 should equal(request)
  }
