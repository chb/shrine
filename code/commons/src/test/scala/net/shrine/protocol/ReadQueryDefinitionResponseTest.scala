 ...
 */
class ReadQueryDefinitionResponseTest extends ShrineResponseI2b2SerializableValidator {
  ...
  val queryDefinition = XmlUtil.stripWhitespace(<ns6:query_definition>
  ...
  @Test
  def testToXml() {
    new ReadQueryDefinitionResponse(masterId, name, userId, createDate, queryDefinition.toString).toXml should equal(readQueryDefinitionResponse)
  }
  ...
  @Test
  def testToI2b2() {
    new ReadQueryDefinitionResponse(masterId, name, userId, createDate, queryDefinition.toString).toI2b2 should equal(response)
  }
}