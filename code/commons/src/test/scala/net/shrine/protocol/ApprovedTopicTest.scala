 ...
 *
 */
final class ApprovedTopicTest extends AssertionsForJUnit with ShouldMatchersForJUnit with XmlSerializableValidator {
  @Test
  def testToI2b2 {
    val id = 123L
    val name = "some-name"

    val i2b2Xml = (new ApprovedTopic(id, name)).toI2b2
  ...
  override def testToXml {
    ...
    val shrineXml = (new ApprovedTopic(id, name)).toXml
  ...
  override def testFromXml {
    ...
    def doTestFromXml(id: Long) {
      ...
      val topic = ApprovedTopic.fromXml(shrineXml)

      topic should not be (null)
      topic.queryTopicId should equal(id)
      topic.queryTopicName should equal(name)

      val roundTripped = ApprovedTopic.fromXml(new ApprovedTopic(id, name).toXml)
