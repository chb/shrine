...
final class ReadApprovedQueryTopicsResponseTest extends ShrineResponseValidator {
  ...
  override val messageBody = <message_body><ns7:sheriff_response xsi:type="ns7:sheriffResponseType" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">{ approvedQueriesResponse.approvedTopics.map(_.toI2b2) }</ns7:sheriff_response></message_body>

  import RandomTool.{randomInt, randomString}

  private def randomApprovedTopic = new ApprovedTopic(randomInt.toLong, randomString)
  ...
  def testToI2b2 {
    ...
    (new ReadApprovedQueryTopicsResponse(approvedQueriesResponse.approvedTopics)).toI2b2 should equal(response)
  }
  ...
  private def shrineXml = <readApprovedQueryTopicsResponse><approvedTopics>{ approvedQueriesResponse.approvedTopics.map(_.toXml) }</approvedTopics></readApprovedQueryTopicsResponse>
