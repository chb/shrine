...
object ObservationResponse extends I2b2Umarshaller[ObservationResponse] with XmlUnmarshaller[ObservationResponse] {
  def fromXml(nodeSeq: NodeSeq) = {

    def checkEmptyString(s: String): Option[String] = {
      if(s.length() > 0) {
        Some(s)
      }
      else {
