...
import net.shrine.webclient.client.widgets.ChartPanelTestGwt;
 ...
 * 
 *      NB: TestSuites are atrocious and error-prone, but using one here
 *      drastically cuts down build times, as firing up the GWT environment
 *      (expensive) only needs to happen once for the suite, not for each test.
 *      :/
 * 
 *      Named GwtTestSuite so the regular Maven surefire plugin won't pick this
 *      up, but the GWT test-running code will.
 */
public final class GwtTestSuite extends GWTTestSuite {
    ...
    public static Test suite() {
        ...
        // NB: all actual tests are named *TestGwt, so the regular Maven
        // surefire plugin
        // won't run them. (It looks for classes with names beginning or ending
        // with 'Test')
        ...
        suite.addTestSuite(ChartPanelTestGwt.class);
