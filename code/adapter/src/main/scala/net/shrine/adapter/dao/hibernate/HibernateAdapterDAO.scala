...
class HibernateAdapterDAO extends AdapterDAO {
  ...
  override def findRequestResponseDataByResultID(resultID: Long): Option[RequestResponseData] = {
    withCurrentSession("Error querying database") { session =>
  ...
  override def findRecentQueries(limit: Int): Seq[UserAndMaster] = {
    withCurrentSession("Error finding recent queries") { session =>
  ...
  private def getAuditEntryEntities(id: Identity): JList[RequestResponseDataEntity] = {
    withCurrentSession("Error getting audit entries") { sessionToUse =>
  ...
  override def isUserLockedOut(id: Identity, defaultThreshold: Int): Boolean = {
    withCurrentSession { session =>
      try {
        val query = session.createQuery("select user.threshold " +
        ...
        val threshold = Option(query.uniqueResult().asInstanceOf[java.lang.Integer]).map(_.intValue).getOrElse(defaultThreshold.intValue)
        ...
        query1.setDate("thirtyDaysAgo", thirtyDaysInThePast)
  ...
  override def findMasterQueryDefinition(broadcastMasterID: Long): MasterQueryDefinition = {
    withCurrentSession("Unexpected Error") { session =>
  ...
  override def removeMasterDefinitions(networkMasterId: Long) {
    withCurrentSession("Error saving object to database") { session =>
