...
case class SquerylQueryResultRow (
  ...
  //NB: For Squeryl, ugh :(
  def this() = this(0, 0L, 0, ResultOutputType.ERROR, QueryResult.StatusType.Error, Option(0L), Util.now)
  
  def toQueryResultRow = QueryResultRow(id, localId, queryId, ResultOutputType.valueOf(resultType).get, QueryResult.StatusType.valueOf(status).get, elapsed, DateHelpers.toXmlGc(lastUpdated))
}