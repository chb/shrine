	...
	this.doQueryRun = function() {
		...
		if (i2b2.PM.model.shrine_domain) {
			...
			if (topicSELECT.selectedIndex == null || topicSELECT.selectedIndex == 0) {
				alert('You must select a Query Topic to run a SHRINE query.');
				return void(0);
			}
		...
		var handleSubmit = function() {
			...
			if(this.submit()) {
				...
				//JIRA|SHRINE-526: Charles McGow 
				//Adding Query Name to html
				i2b2.CRC.ctrlr.QT.doSetQueryName(queryNameInput.value);
				//JIRA|SHRINE-526: Charles McGow
				var options = {};
...
// ================================================================================================== //
	this._queryRun = function(inQueryName, options) {
		...
		if (i2b2.h.isSHRINE()) {
			var topicSELECT = $('queryTopicSelect');
			if (topicSELECT.selectedIndex == null || topicSELECT.selectedIndex == 0) {
				alert("Please select a Query Topic to run the query.");
				return false;
			}
