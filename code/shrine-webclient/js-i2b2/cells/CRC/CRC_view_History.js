...
i2b2.CRC.view.history.refreshPrevQueries = function(subScreen){
	$("refPrevQS").setStyle({
		display:'none'
	});
	$("refPrev2QS").setStyle({
	...
	i2b2.CRC.ctrlr.history.SyncRefresh();
...
i2b2.CRC.view.history.PopulateQueryMasters = function(dm_ptr, dm_name, options) {
	...
	if(useSorting){
		...
		sortFinal = i2b2.CRC.model.QueryMasters.sortBy(QM_sortVal);
		
		// reverse if needed
		if (reverseSort) { sortFinal.reverse(true); }
	}
	else{
		sortFinal = i2b2.CRC.model.QueryMasters.toArray();
	}
