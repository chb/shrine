...
// ================================================================================================== //
	doNameSearch: function(inSearchData) {
		...
		for (var i=0; i<l; i++) {
			...
			//JIRA|SHRINE-328: Charles McGow
			//MAX_EXCEEDED
			
			v_status = 
				i2b2.h.XPath(results.refXML, '//result_status/status')[0];
			...
			if(str_status.search(/ERROR/i) > -1){
				if(str_status_message == "MAX_EXCEEDED"){
					//Too many concepts returned for search
					alert("Your search results have exceeded the maximum-number allowed. "+
						  "Please add additional terms to narrow your query.");
					break;
				}
				else{
					//Different error received
			...
			//JIRA|SHRINE-328: Charles McGow
				...
				for(var i2=0; i2<1*c.length; i2++) {
					...
					var renderOptions = {
						title: o.name,
						//JIRA|SHRINE-455: Charles McGow
						cd_tooltip: o.tooltip,
						//JIRA|SHRINE-455: Charles McGow						
...
// ================================================================================================== //
	doCodeSearch: function(inSearchData) {
		...
		searchOptions.ont_synonym_records = i2b2.ONT.view['find'].params.synonyms;
		searchOptions.ont_hidden_records = i2b2.ONT.view['find'].params.hiddens;
		searchOptions.ont_search_strategy = "left";
