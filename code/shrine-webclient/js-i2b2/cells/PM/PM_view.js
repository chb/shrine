...
i2b2.PM._redrawConnectedForm = function() {
	...
	if((dml == null) || (dml.length == 0 )){
		   alert('No Hostnames available!');
	}
	else{
		//Allows user to select host for list of hosts.
	...
	//If only one host should be hidden.
	if((dml != null) && (dml.length == 1)){
		//No need to host information.
		$('shrine_host_label').hide();
		$('shrine_host_opt_input').hide();
	}
	//JIRA|SHRINE-527:Charles McGow
}
