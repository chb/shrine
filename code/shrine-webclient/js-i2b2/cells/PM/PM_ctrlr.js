...
i2b2.PM._processUserConfig = function (data) {
	...
	/* JIRA|SHRINE-356:Charles McGow */
	v_status = 
		i2b2.h.XPath(data.refXML, '//result_status/status')[0];
		
	str_status =
		i2b2.h.Xml2String(v_status);
	
	str_status_message =
		i2b2.h.getXNodeVal(v_status, 'status');
	/* JIRA|SHRINE-356:Charles McGow */
	...
	try {
		...
		console.error("Could not find returned password in login XML");
	...
	for (var i=0; i<projs.length; i++) {
		...
		for (var d=0; d<projdetails.length; d++) {
			...
			if (projdetails[d].textContent) {
				i2b2.PM.model.projects[code].details[paramName] = projdetails[d].textContent.unescapeHTML();
			} else {
	...
	/* JIRA|SHRINE-356:Charles McGow */
	if((str_status == "<status type=\"ERROR\">Logon failure: unknown user name or bad password.</status>") ||
	   (str_status == "<status type=\"ERROR\">Username or password does not exist</status>") ||
	   (str_status == "<status type=\"ERROR\">Username does not exist</status>") ||
	   (str_status == "<status type=\"ERROR\">Supplied password does not match user password!</status>")
	){
		try { i2b2.h.LoadingMask.hide(); } catch(e) {}
		alert("Invalid logon! Please try again! ");
		try { i2b2.PM.view.modal.login.show(); } catch(e) {}
		return true;
	}
	else if(str_status.search(/ERROR/i) > -1){
		try { i2b2.h.LoadingMask.hide(); } catch(e) {}
		
		alert(str_status_message);
		try { i2b2.PM.view.modal.login.show(); } catch(e) {}
		return true;
	}
...
i2b2.PM._processLaunchFramework = function() {
	...
	// add a convenience method to the roles array
	i2b2.PM.model.userRoles.contains = function(obj) {
		var i = this.length;
		while (i--) {
			if (this[i] === obj) {
				return true;
