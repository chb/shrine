...
class ShrineService(
  ...
  protected def executeRequest(identity: Identity, message: BroadcastMessage, aggregator: Aggregator): ShrineResponse = {
    val ackNack = broadcastMessage(message,
      new QueryInfo(determinePeergroup(message.request.projectId), identity, message.request.requestType.name, endpointConfig))
  ...
  private def auditRunQueryRequest(identity: Identity, request: RunQueryRequest) {
    auditDao.addAuditEntry(new AuditEntry(
      request.projectId,
      identity.getDomain,
      identity.getUsername,
      request.queryDefinitionXml,
      request.topicId))
  }
  ...
  def runQuery(request: RunQueryRequest) = {
    ...
    val aggregator = new RunQueryAggregator(message.masterId.get, request.authn.username, request.projectId,
      request.queryDefinitionXml, message.instanceId.get, shrineConfig.isIncludeAggregateResult)
