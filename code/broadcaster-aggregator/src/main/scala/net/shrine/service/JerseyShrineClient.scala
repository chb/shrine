...
final class JerseyShrineClient(val shrineUrl: String, val projectId: String, val authorization: AuthenticationInfo, val acceptAllSslCerts: Boolean = false) extends ShrineClient {
  ...
  override def runQuery(topicId: String, outputTypes: Set[ResultOutputType], queryDefinitionXml: String) = {
    post[RunQueryResponse] {
      webResource.path("/shrine/queries").header("outputTypes", OutputTypeSet(outputTypes).serialized).header("topicId", topicId).entity(queryDefinitionXml, MediaType.APPLICATION_XML)
    }
  }
