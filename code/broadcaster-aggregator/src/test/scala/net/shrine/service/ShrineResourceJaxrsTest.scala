...
final class ShrineResourceJaxrsTest extends JerseyTest with AssertionsForJUnit with ShouldMatchers {
  ...
  def testRunQuery = resetMockThen {
    ...
    def doTestRunQueryResponse(response: RunQueryResponse, expectedOutputTypes: Set[ResultOutputType]) {
      ...
      param.queryDefinitionXml should equal(queryDefXml)
      param.topicId should equal(topicId)
    }

    def doTestRunQuery(outputTypes: Set[ResultOutputType]) {
      val responseScalaSet = shrineClient.runQuery(topicId, outputTypes, queryDefXml)

      doTestRunQueryResponse(responseScalaSet, outputTypes)

      import scala.collection.JavaConverters._

      val responseJavaSet = shrineClient.runQuery(topicId, outputTypes.asJava, queryDefXml)
  ...
  private object MockShrineRequestHandler extends ShrineRequestHandler {
    ...
    def readInstanceResults(request: ReadInstanceResultsRequest): ShrineResponse = {
      ...
      new ReadInstanceResultsResponse(1337L, Seq(new QueryResult(123L, 1337L, "resultType", 789L, None, None, Some("description"), "statusType", Some("statusMessage"))))
    }
    ...
    def runQuery(request: RunQueryRequest): ShrineResponse = {
      ...
      new RunQueryResponse(123L, now, "userId", "groupId", request.queryDefinitionXml, 456L, Seq.empty)
    }
