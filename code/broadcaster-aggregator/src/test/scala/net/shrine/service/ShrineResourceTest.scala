...
final class ShrineResourceTest extends TestCase with AssertionsForJUnit with ShouldMatchersForJUnit with EasyMockSugar {
  ...
  def testRunQuery {
    ...
    val expectedRequest = new RunQueryRequest(projectId, waitTimeMs, authenticationInfo, topicId, outputTypes, queryDef)
    val expectedResponse = new RunQueryResponse(0L, null, "userId", "groupId", queryDef, 0L, Seq.empty)
