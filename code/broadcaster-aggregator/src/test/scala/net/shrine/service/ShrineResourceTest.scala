...
import org.easymock.EasyMock.{ eq => isEqualTo, expect => invoke }
...
final class ShrineResourceTest extends TestCase with AssertionsForJUnit with ShouldMatchersForJUnit with EasyMockSugar {
  ...
  def testRunQuery {
    ...
    val queryDefinitionXml = "queryDefinitionXml"
    val topicId = "topicId"

    val expectedRequest = new RunQueryRequest(projectId, waitTimeMs, authenticationInfo, topicId, outputTypes, queryDefinitionXml)
    val expectedResponse = new RunQueryResponse(0L, null, "userId", "groupId", "requestXml", 0L, Seq.empty)

    setExpectations(_.runQuery, expectedRequest, expectedResponse)

    execute {
      resource.runQuery(projectId, authenticationInfo, topicId, new OutputTypeSet(outputTypes), queryDefinitionXml)
    }
  }
