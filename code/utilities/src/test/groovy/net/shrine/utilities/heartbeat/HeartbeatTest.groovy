...
class HeartbeatTest
{
  ...
  {
    setUpSql()
    heartbeat = new Heartbeat([expectedNumberOfNodes: 3, failureThreshold: 2], sql)
  }
  ...
  {
    sql.executeInsert("insert into EMAIL_NOTIFICATION VALUES(DATEADD('DAY', -1, CURRENT_TIMESTAMP()))") //this only works on H2 dbs but we only need this SQL for testing
    assertFalse heartbeat.emailSentToday()
    sql.executeUpdate "update EMAIL_NOTIFICATION set LAST_NOTIFICATION=CURRENT_TIMESTAMP()"
    assertTrue heartbeat.emailSentToday()
  }
